...
object Huffman {
  ...
  def decode2(tree: CodeTree, bits: List[Bit]): Stream[Char] = {
    def traverse(t: CodeTree, bits: List[Bit]): Stream[Char] = t match {
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def encodeSymbol(t: CodeTree)(ch: Char): List[Bit] = t match {
      case Leaf(_, _) => Nil
